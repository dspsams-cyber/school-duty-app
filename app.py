...
class DutyScheduler:
    ...
    def run_scheduler(self, week_type):
        ...
        for duty, details in sorted_duties:
            day = duty.split('_')[0]
            slot = self._get_duty_slot(duty) # 抓取該崗位的時段
            assigned = []
            ...
            for teacher in assigned:
                mark_busy(teacher, day, slot) # 【新增】指派後立刻佔用該時段
...
st.markdown("系統已加入**同日同時段防分身機制**，確保所有老師不會被安排在同一時間執行多個崗位。")
st.divider()
